...
const checkEnv = async () => {
  try {
    ...
    if (pythonStatus === EnvStatus.PythonNotInstalled) {
      ...
      steps.value[0].description = '环境未安装'
    ...
    steps.value[0].loading = false
    steps.value[0].description = '环境已安装'
    ...
    if (status === EnvStatus.RemBGNotInstalled) {
      ...
      steps.value[1].description = '依赖未安装'
      steps.value[2].status = 'error'
      steps.value[2].description = '配置未完成'
    } else {
      steps.value[1].status = 'finish'
      steps.value[1].loading = false
      steps.value[1].description = '依赖已安装'

      // 系统配置检查
      await new Promise((resolve) => setTimeout(resolve, 500))
      currentStep.value = 2
      steps.value[2].status = 'finish'
      steps.value[2].description = '配置完成'
      setTimeout(() => {
        emit('env-ready')
      }, 1000)
    }
